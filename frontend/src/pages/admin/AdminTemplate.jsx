...
import { Image as ImageIcon, Send, XCircle, CheckCircle, Upload, Users, Link2, ExternalLink, Type, MousePointer2 } from 'lucide-react';
...
const AdminTemplate = () => {
  ...
  const fetchTemplate = React.useCallback(async () => {
    try {
      ...
      setButtonUrl(response.data.global_button_url || '');
              ...
              <div style={{ display: 'flex', gap: '0.75rem', flexDirection: 'column', background: 'rgba(255,255,255,0.02)', padding: '1rem', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
                <div style={{ position: 'relative' }}>
                   ...
                   <input 
                    ...
                    placeholder="Teks Tombol (Contoh: Cek Info, Download, Daftar...)"
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div style={{ background: '#e1eec3', backgroundImage: 'linear-gradient(to top, #f05053 0%, #e1eec3 100%)', borderRadius: '16px', padding: '1.5rem', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)', position: 'relative' }}>
            ...
            <div style={{ background: 'var(--card-bg)', borderRadius: '8px', overflow: 'hidden', boxShadow: '0 4px 6px rgba(0,0,0,0.05)' }}>
              ...
              {/* Button — always shown in preview, greyed when empty */}
              <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', margin: '0 0.75rem', paddingBottom: '0.75rem' }}>
                <div style={{ 
                  display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', 
                  color: buttonText ? '#34d399' : '#475569',
                  fontWeight: 700, fontSize: '0.85rem', 
                  background: buttonText ? 'rgba(52, 211, 153, 0.1)' : 'rgba(255,255,255,0.03)',
                  padding: '0.6rem', borderRadius: '8px',
                  border: `1px solid ${buttonText ? 'rgba(52, 211, 153, 0.25)' : 'rgba(255,255,255,0.06)'}`,
                  transition: 'all 0.3s'
                }}>
                  <ExternalLink size={14} /> {buttonText || 'Belum ada teks tombol...'}
                </div>
              </div>
            </div>
          </div>
